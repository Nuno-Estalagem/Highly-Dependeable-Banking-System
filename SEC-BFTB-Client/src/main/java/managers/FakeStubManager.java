...
public class FakeStubManager {
    ...
    public FakeStubManager(StubHandler stubHandler, int byzantineQuorum, int byzantineServers, HashMap<String, BTFTBGrpc.BTFTBBlockingStub> stubs) {
    ...
    public Object generateMethod(String input, List<Object>requested, Client cliente) {
        try {
            int bizantinos = 0;
            ...
            if (input.equals("open")) {
                List<BFTB.OpenAccountRequest> requests= (List<BFTB.OpenAccountRequest>) requested.get(0);
            ...
            if (input.equals("check")) {
                List<BFTB.CheckAccountRequest> requests= (List<BFTB.CheckAccountRequest>) requested.get(0);
                CountDownLatch countDownLatch = new CountDownLatch(this.byzantineQuorum);
                int i=-0;
                for (String key : keys) {
                    Object request=requests.get(i);
                ...
                synchronized (outputs) {
                    ...
                    int j=0;
                    for (PublicKey serverKey : serverKeys) {
                        BFTB.CheckAccountRequest accountRequest = (BFTB.CheckAccountRequest) requests.get(j);
                        j++;
            ...
            if (input.equals("audit")) {
                List<BFTB.AuditRequest> requests= (List<BFTB.AuditRequest>) requested.get(0);
                CountDownLatch countDownLatch = new CountDownLatch(this.byzantineQuorum);
                int i=0;
                for (String key : keys) {
                    Object request=requests.get(i);
                ...
                synchronized (outputs) {
                    ...
                    for (PublicKey serverKey : serverKeys) {
                        j++;
                        BFTB.AuditRequest auditRequest = (BFTB.AuditRequest) requests.get(j);
            ...
            if (input.equals("receive")) {
                List<BFTB.ReceiveAmountRequest> requests= (List<BFTB.ReceiveAmountRequest>) requested.get(0);
                int i=0;
                ...
                synchronized (outputs) {
                    ...
                    int j=0;
                    for (PublicKey serverKey : serverKeys) {
                        int receiveWid = ((BFTB.ReceiveAmountRequest) requests.get(j)).getWid();
                        j++;
            ...
            if (input.equals("send")) {
                List<BFTB.SendAmountRequest> requests= (List<BFTB.SendAmountRequest>) requested.get(0);
                int i=0;
                CountDownLatch countDownLatch = new CountDownLatch(this.byzantineQuorum);
                for (String key : keys) {
                    Object request=requests.get(i);
            ...
            if (input.equals("rid")) {
                List<BFTB.GetRidRequest> requests= (List<BFTB.GetRidRequest>) requested.get(0);
                int i=0;
                CountDownLatch countDownLatch = new CountDownLatch(this.byzantineQuorum);
                for (String key : keys) {
                    Object request=requests.get(i);
            ...
            if (input.equals("CAWB")) {
                int i=0;
                List<BFTB.CAWBRequest> requests= (List<BFTB.CAWBRequest>) requested.get(0);
                CountDownLatch countDownLatch = new CountDownLatch(this.byzantineQuorum);
                for (String key : keys) {
                    Object request=requests.get(i);
                    i++;
            ...
            if (input.equals("ADWB")) {
                int i=0;
                List<BFTB.ADWBRequest> requests= (List<BFTB.ADWBRequest>) requested.get(0);
                CountDownLatch countDownLatch = new CountDownLatch(this.byzantineQuorum);
                for (String key : keys) {
                    Object request=requests.get(i);
                    i++;
