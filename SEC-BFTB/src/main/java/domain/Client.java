...
public final class Client {
    ...
    private Client(String path, String keystore, String keystorePass, String alias) throws NumberFormatException, IOException, KeyStoreException, NoSuchAlgorithmException, CertificateException, UnrecoverableKeyException {
        ...
        String regexCer = "server_[0-9][0-9][0-9][0-9].cer";
        String regex = "[0-9][0-9][0-9][0-9]";
        Pattern p = Pattern.compile(regex);
        Pattern p2 = Pattern.compile(regexCer);
        File dir = new File(path);
        if (dir.isDirectory()) {
            File[] ficheiros = dir.listFiles((File file) -> p.matcher(file.getName()).matches());
            for (File file : ficheiros) {
                if (file.isDirectory()) {
                    File[] certificados = file.listFiles((File f) -> p2.matcher(f.getName()).matches());
                    for(File certificado: certificados)
                        serverPubKeys.put(certificado.getPath().split("_")[1].split("\\.")[0], CipherHandler.getCertificateFromTrustStore(certificado.getPath()).getPublicKey());
                }
            }
        }
